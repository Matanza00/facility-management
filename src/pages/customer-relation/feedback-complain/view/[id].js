...
{Object.entries(complainData || {})
  .filter(([key]) => key !== 'tenant' && key !== 'deletedAt')
  .map(([key, value]) => {
    // Ensure you're rendering primitive types or strings
    if (['createdAt', 'updatedAt'].includes(key)) {
      ...
      return (
        <div key={key} className="text-sm">
          ...
          {date.toISOString().split('T')[0]} Time: {date.toTimeString().split(' ')[0]}
        </div>
      );
    }
              ...
              <tbody>
                {slips.map((slip) => (
                  <tr key={slip.id} className="hover:bg-gray-50">
                    ...
                    <td className="border px-4 py-2">{slip.attendedBy.split(',').join(', ')}</td>
...
{(session?.user.role.toLowerCase() === 'super_admin' || session?.user.role.toLowerCase() === 'supervisor'|| session?.user.role.toLowerCase() === 'bookkeeper') && (
        ...
        {(session?.user.role.toLowerCase() === 'super_admin' || session?.user.role.toLowerCase() === 'supervisor') && (
