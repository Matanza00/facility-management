...
const EditBoilerForm = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            ShutdownTime: new Date(data.ShutdownTime).toISOString().slice(0, 16),
            Remarks: data.Remarks || '',
            OperatorName: data.OperatorName || '',
            SupervisorName: data.SupervisorName || '',
            Chillers: data.Chillers.map((chiller) => ({
              ...chiller,
              time: new Date(chiller.time).toISOString().slice(11, 16), // Extract time
            })) || [],
          });
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Boiler updated successfully');
        router.push(`/daily-maintenance/absorptionchiller/view/${id}`);
      } else {
        alert('Error updating boiler');
      }
    } catch (error) {
  ...
  return (
    <Layout>
      <div className="p-6 bg-gray-50 min-h-screen">
        <h1 className="text-3xl font-semibold mb-8 text-gray-800">Edit Absorption Chiller</h1>
