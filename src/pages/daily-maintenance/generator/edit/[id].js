...
export default function EditGenerator() {
  ...
  useEffect(() => {
    const fetchGeneratorData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const electricianMatch = electricians.find((el) => el.name === data.electricianName);
          const engineerMatch = engineers.find((en) => en.name === data.engineerName);
    ...
    const fetchDropdownData = async () => {
      try {
        const electricianRes = await fetch('/api/users/filtered?roles=Technician&departments=Electrical');
        const engineerRes = await fetch('/api/users/filtered?roles=Supervisor&departments=Electrical');
        ...
        // Fetch generator data after dropdowns to ensure matching
        fetchGeneratorData();
      } catch (error) {
