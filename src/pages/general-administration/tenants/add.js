...
export default function AddTenant() {
  ...
  useEffect(() => {
    const fetchTenants = async () => {
      try {
        const response = await fetch("http://localhost:3000/api/users/filtered?roles=tenant");
        if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    const tenantData = {
      tenantName: selectedTenantId, // Send tenant ID
    ...
    <Layout>
      <div className="max-w-3xl mx-auto p-8 bg-white shadow-lg rounded-lg">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Total Area */}
          <div className="mb-6">
            <label className="block text-lg font-medium text-gray-700">Total Area (sq. meters)</label>
            <input
              type="number"
              value={totalAreaSq}
              disabled
          ...
          {areas.map((area, index) => (
            <div key={index} className="space-y-4 mb-6">
              ...
              <div className="flex gap-4">
                {/* Occupied Area */}
                <div className="flex-1">
                  <label className="block text-lg font-medium text-gray-700">Occupied Area (sq. meters)</label>
                  <input
                    type="number"
