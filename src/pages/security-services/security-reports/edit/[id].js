    ...
    const fetchDropdownData = async () => {
      try {
        const [techniciansRes, supervisorsRes] = await Promise.all([
          fetch('/api/users/filtered?roles=Technician&departments=Security'),
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedData = {
      ...
      timeNoted: formData.timeNoted
        ? new Date(`${formData.date}T${formData.timeNoted}:00`).toISOString()
        : null,
      timeSolved: formData.timeSolved
        ? new Date(`${formData.date}T${formData.timeSolved}:00`).toISOString()
        : null,
    };
    ...
    return {
      props: {
        report: {
          ...
          timeNoted: report.timeNoted ? report.timeNoted.toISOString().split('T')[1]?.slice(0, 5) : '',
          timeSolved: report.timeSolved ? report.timeSolved.toISOString().split('T')[1]?.slice(0, 5) : '',
          observedBy: report.observedBy,
          supervisor: report.supervisor,
          description: report.description,
          action: report.action,
